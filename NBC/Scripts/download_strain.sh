...
start=`date +%s`
java -jar camiClient.jar -d /lustre/scratch/yy476/Naive_Bayes/tools/Strain_Madness_Dataset_Assembly strainmadness/assembly -p fq.gz
java -jar camiClient.jar -d /lustre/scratch/yy476/Naive_Bayes/tools/Strain_Madness_Dataset_Raw_Data strainmadness/raw -p fq.gz
end=`date +%s`
runtime=$((end-start))
echo "updating time is $runtime"
